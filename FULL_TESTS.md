...
| EDIT-010 | Type a draft, press `Down`, press `Up`, then type and execute a different command. | `Down` saves the first draft and starts a clean new draft; `Up` restores the saved draft; the later command does not append to stale draft text. | Paste output and draft JSONL evidence. |
