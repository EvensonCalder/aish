...
- For non-first-token completion with backend shell completion enabled, lower-priority Aish history/template candidates may be held during a bounded backend-priority window while the backend shell query is pending. This prevents `Tab` from accepting a similar history argument before a native shell subcommand or option candidate can arrive, without indefinitely blocking Aish fallback completion if the shell query is slow. Local path candidates and first-token executable candidates must remain usable while the shell query is pending.
- Backend shell completion should query the configured shell's own completion system instead of migrating shell completion files into Aish-specific data. Fish completion should use fish `complete -C`; bash completion should execute programmable completion functions/compspecs from an interactive bash; zsh completion should query an interactive zsh completion session. User config such as fish `~/.config/fish/completions`, fish interactive `config.fish`, bash `complete` definitions loaded from `.bashrc`, bash-completion scripts, zsh `fpath`, zsh `compinit`/`compdef` mappings loaded from `.zshrc`, zsh `$ZDOTDIR/completions`, zsh `$ZDOTDIR/.zsh/completions`, and zsh `~/.zsh/completions` should work. Zsh fallback completion directories should be target-loaded for the current command even when user startup already ran `compinit`.
...
- When the current token is path-like, structural history/template candidates must still match that path prefix before they can be displayed or accepted. This prevents an unrelated same-command history suffix from taking priority over an explicit filesystem path prefix such as `src/m`.
...
- Pressing `Tab` with no visible candidates and an empty pending live result should replace that pending request with an explicit request whose backend shell query is not delayed by the ordinary live-completion debounce.
- Pressing `Tab` while a non-first-token backend shell query is still inside the bounded backend-priority window must not accept a lower-priority Aish history/template candidate. When the backend query returns candidates, those candidates are ranked first; when it returns empty or the priority window expires, the deferred Aish candidates may be displayed or accepted normally.
...
- Live completion is layered: cheap local path candidates can be found immediately, template/history/PATH executable results can refresh the UI when the worker finishes, backend shell completion can refresh the UI independently after a short debounce, and slower typo-correction results can refresh the UI after that when `completion.fuzzy = true`.
...
- If the first-token immediate result set contains only PATH executable candidates and an async history tier is pending, Aish may defer drawing those executable candidates until the coalescing window resolves. Backend shell completion must not defer or block already available local path or first-token executable candidates; when shell candidates arrive later, they are merged, ranked first, and deduplicated without invalidating the current input. Non-first low-priority Aish history/template candidates may be deferred during the bounded backend-priority window so shell-native completions keep first position without wedging fallback completion.
