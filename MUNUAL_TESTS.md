...
| H-008 | Run full-screen or interactive passthrough programs such as `vim`, `nvim`, `top`, `python`, `node`, `ssh`, or nested `tmux`. Use their normal keybindings, then exit. | Aish forwards keys to the program and recovers its prompt after the program exits. | `less` smoke is automated; broad alternate-screen and job-control behavior is not portable enough for one tmux assertion. |
...
| H-010 | When GPG-backed storage is implemented, run `#key set`, `#key clear`, `#encrypt on`, and `#encrypt off` with an isolated test key and pinentry. | GPG/pinentry gets terminal control safely, secrets are not printed, encrypted files are written, and plaintext storage claims are accurate. | Encryption is currently a safe placeholder; real GPG/pinentry interaction needs a dedicated future test path. |
