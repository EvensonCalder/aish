...
- Backend shell completion has Rust coverage for fish `~/.config/fish/completions`, fish interactive `config.fish`, bash `.bashrc` programmable completion, common bash compspec flags, zsh `fpath`/`~/.zsh/completions`, backend-first dedupe, and live UI deferral before lower-priority local path candidates.
