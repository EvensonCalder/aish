...
- Context configuration persists `#context on|off`, `#context confirm on|off`, and `#context <bytes>` to `config.toml`; context command execution remains disabled until the execution/confirmation flow is implemented.
