...
- Backend shell completion has Rust coverage for fish `~/.config/fish/completions`, fish interactive `config.fish`, bash `.bashrc` programmable completion, common bash compspec flags, zsh `fpath`/`~/.zsh/completions`, backend-first dedupe, nonblocking local candidate display before slow shell results, and Tab completion while a slow backend shell query is pending.
