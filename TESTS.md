...
- Backend shell completion has Rust coverage for fish `~/.config/fish/completions`, fish interactive `config.fish`, bash `.bashrc` programmable completion, common bash compspec flags, zsh `fpath`/`compinit`/`compdef` mappings from `.zshrc`, zsh `$ZDOTDIR/completions`/`$ZDOTDIR/.zsh/completions`/`~/.zsh/completions`, cancellable backend queries, backend-first dedupe, nonblocking local candidate display before slow shell results, and Tab completion while a slow backend shell query is pending.
