...
- `#completion` reports current completion config and persists `#completion max <count>`, `#completion inline on|off`, `#completion backend on|off`, and `#completion tab-accept full|word`.
...
- Backend shell completion has Rust coverage for fish `~/.config/fish/completions`, fish interactive `config.fish`, bash `.bashrc` programmable completion, common bash compspec flags, zsh `fpath`/`compinit`/`compdef` mappings from `.zshrc`, zsh `$ZDOTDIR/completions`/`$ZDOTDIR/.zsh/completions`/`~/.zsh/completions` including target-loading after user `compinit`, cancellable backend queries, backend-first dedupe, explicit Tab priority backend requests, empty backend completion completion-events, nonblocking local path/executable display before slow shell results, deferred history/template display during the bounded backend-priority window, fallback history acceptance after that window expires, path-like structural history filtering, and Tab not accepting a lower-priority history candidate while a backend shell query is still inside the priority window. Real shell/PTY unit tests use an explicit process-wide and cross-process lock so zsh/fish/bash helper sessions do not race each other for PTY and startup resources.
...
- Completion helpers support ignore-spaces matching and panel max-result limiting; config defaults expose `completion.max_results = 5`, `completion.backend = true`, `completion.ignore_spaces = true`, `completion.template_first = true`, `completion.inline = true`, and `completion.tab_accept = "word"`.
