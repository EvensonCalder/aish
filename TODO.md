...
- [x] Start encrypted history/template unlock asynchronously on startup without invoking pinentry in the background.
- [x] Show `history is still unlocking...` when needed.
- [x] Temporarily leave raw mode for direct GPG/pinentry decrypt operations.
- [x] Handle direct GPG/pinentry decrypt and rewrite operations through the UnlockPassthrough state.
  - [x] Give GPG/pinentry dedicated terminal control for direct decrypt/unlock operations.
...
- Aish remains usable while decrypting history/templates; passphrase-required startup unlock is explicit through `#unlock`.
...
- [x] Match path completion against shell word values and insert shell-safe path replacements for unquoted, single-quoted, double-quoted, and backslash-escaped typed paths, including shell-compatible leading `~/` handling.
...
- [x] Evaluate a span-first completion protocol. Aish now materializes accepted candidates as explicit `CompletionEdit { start, end, replacement }` values before applying them to the draft. Candidate generation can still stay lightweight, while acceptance no longer buries replacement ranges inside string-splicing code.
- [x] Evaluate richer semantic completion candidates. The current model keeps replacement text, display text, source/kind, directory state, and accepted edit ranges separated. Append-space behavior, numeric scores, and match indices are deferred until rendering or ranking needs them; adding them now would mostly add churn without improving Aish's PTY-shell boundary.
...
- [x] Evaluate a dedicated matcher abstraction for structural prefix matching, typo correction, and path component matching. Aish now has a small `CompletionMatcher` that centralizes structural prefix, word-threshold, and typo-threshold semantics. Path component matching keeps its extra directory-specific rules, but uses the same threshold meanings.
- [x] Evaluate and absorb the practical subset of component-wise path completion inspired by Nushell's path handling. Aish now keeps its shell-safe Bash/Zsh/Fish insertion rules while resolving missing intermediate directory components through exact-directory descent, directory-prefix matches, fuzzy directory typo correction, leading `~/` handling, and visible-before-hidden ordering. Match indices and broader platform-prefix modeling are deferred to the richer candidate/edit-range design.
- [x] Evaluate a more explicit conservative continuation lexer. Aish already uses a stable lexical continuation check for single quotes, double quotes, and escaped trailing newlines, with tests proving it does not treat shell-language blocks such as `if true` as Aish-owned continuations. Obvious unmatched delimiters remain deferred because they risk overriding backend shell semantics.
- [x] Evaluate keybinding diagnostics inspired by Nushell, such as listing effective keybindings or listening for a key sequence. Aish already exposes effective action bindings through `#help keys`; an interactive key listener is deferred because it would add a new raw-input mode without a current diagnostic gap.
- [x] Do not absorb Nushell's typed pipeline, AST execution model, dynamic completion closures, or full shell language semantics into Aish. This remains an explicit boundary: Aish may borrow small completion data-shape ideas, but backend shells retain parsing and execution ownership.
