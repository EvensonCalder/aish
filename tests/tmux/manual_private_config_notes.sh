...
HOME_DIR="/tmp/aish-tmux-manual-private-home-$$"
...
printf '%s\n' "$CAPTURE" | rg -q '^#key set is not implemented yet; no key stored$'
printf '%s\n' "$CAPTURE" | rg -q '^no stored key to clear$'
printf '%s\n' "$CAPTURE" | rg -q '^encryption is not implemented yet; no data changed$'
