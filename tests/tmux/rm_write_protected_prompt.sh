...
printf '[shell]\nbackend = "%s"\n' "$BACKEND_SHELL" > "$HOME_DIR/.aish/config.toml"
touch "$WORK_DIR/1.t"
chmod 400 "$WORK_DIR/1.t"

tmux new-session -d -x 80 -y 10 -c "$WORK_DIR" -s "$SESSION" "env LC_ALL=C LANG=C SHELL='$BACKEND_SHELL' HOME='$HOME_DIR' AISH_HOME='$HOME_DIR/.aish' '$AISH_BIN'"
sleep 5
