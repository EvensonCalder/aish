...
HOME_DIR="${AISH_TMUX_ARTIFACT_DIR:-/tmp}/aish-tmux-template-completion-home-$$"
...
tmux send-keys -t "$SESSION" Tab
sleep 1
STRUCTURAL_WORD_CAPTURE="$(tmux capture-pane -p -t "$SESSION")"
...
printf '%s\n' "$STRUCTURAL_WORD_CAPTURE" | rg -q 'echo \{a\} \{b\}'
