...
pub(super) fn template_usage() -> &'static str {
    "usage: #template search <query> | #template list [>|>> <path> | | <command>] | #template find <query> | #template show <id> | #template use <id> [key=value...] | #template rm <id> | #template replace <id> <body>"
}
