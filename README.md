...
- Pressing `Tab` explicitly restarts an empty live request as a priority backend-shell completion request, so shell completions that only produce choices from a native Tab action can still appear without waiting for the ordinary live-completion debounce.
