...
GPG-backed key storage, encrypted history/templates, and git sync are not implemented yet. Current `#key`, `#encrypt`, `#set-remote`, `#push`, and `#sync` commands are safe placeholders and do not mutate secrets, encryption state, remotes, or git repositories.
