...
- Structural history/template matches use `completion.match_threshold_percent` as a word-position match rate. The default is `50`, so one matching word out of two typed words is enough. When the current token is path-like, structural candidates must still match that path prefix so an unrelated history command cannot displace local path completion.
...
- For non-first-token completion with `completion.backend=true`, low-priority Aish history/template candidates may be held briefly while the backend shell query is pending so native subcommands and option completions can occupy the inline slot first. The wait is bounded; local path candidates and first-token executable completion remain usable while the shell query is pending.
- Pressing `Tab` explicitly restarts an empty live request as a priority backend-shell completion request, so shell completions that only produce choices from a native Tab action can still appear without waiting for the ordinary live-completion debounce. While that backend query is inside its bounded priority window, `Tab` will not accept a lower-priority history/template candidate ahead of the shell candidate.
- Backend shell completion is queried from the real configured shell instead of by parsing completion files. Fish uses `complete -C`; bash runs interactive programmable completion functions and compspecs; zsh runs an interactive completion query in a PTY. This lets normal shell config such as `~/.config/fish/completions`, bash `complete`/bash-completion loaded from `.bashrc`, zsh `fpath`, zsh `compdef` mappings loaded from `.zshrc`, `$ZDOTDIR/completions`, `$ZDOTDIR/.zsh/completions`, and `~/.zsh/completions` work through the shell's own completion engine.
...
Backend shell completion follows the configured backend shell. Aish does not migrate fish, zsh, or bash completion files into its own format; it asks the shell to evaluate them. Zsh completion queries run in an isolated interactive zsh PTY, first try the fast shell completion environment, fall back to sourcing the user's `.zshrc` when needed, preserve existing `compdef` mappings, and also target-load matching `_command` files from `$ZDOTDIR/completions`, `$ZDOTDIR/.zsh/completions`, and `~/.zsh/completions`. Fish completion files under `~/.config/fish/completions` and completions registered from interactive `config.fish` are loaded through fish. Bash completions registered from `.bashrc` or bash-completion scripts are executed through bash programmable completion.
